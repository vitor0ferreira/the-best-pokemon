...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 w-full backdrop-blur-xl bg-obsidian/85 border-b border-slate-200 dark:border-white/10 transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-2.5 group focus:outline-none">
          ...
          <div className="flex flex-col">
            <span className="font-black text-lg md:text-xl tracking-tight text-slate-900 dark:text-white flex items-center gap-1">
        ...
        {/* Desktop Navigation */}
        <nav className="hidden sm:flex items-center gap-1 bg-slate-200/60 dark:bg-slate-900/60 p-1.5 rounded-full border border-slate-300/40 dark:border-white/10">
          {navLinks.map((link) => {
            ...
            return (
              <Link
                ...
                className={`relative px-4 py-1.5 rounded-full text-sm font-semibold transition-all flex items-center gap-2 ${
                  isActive
                    ? 'text-white'
                    : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 hover:bg-slate-300/40 dark:hover:bg-white/5'
                }`}
              >
                {isActive && (
                  <motion.div
        ...
        {/* User Dropdown, Language Selector, Theme Toggle & Mobile Menu Button */}
        <div className="flex items-center gap-2 sm:gap-2.5">
          ...
          <button
            ...
            className="sm:hidden p-2 rounded-xl bg-slate-200 dark:bg-slate-800 border border-slate-300/60 dark:border-white/10 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-colors"
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="sm:hidden border-b border-slate-200 dark:border-white/10 bg-obsidian-surface px-4 py-4 space-y-2 overflow-hidden"
          >
            {navLinks.map((link) => {
              const isActive = pathname === link.href;
              const Icon = link.icon;
