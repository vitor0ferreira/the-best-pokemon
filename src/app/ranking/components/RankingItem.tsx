...
function RankingItem({ id, pokemon, rank, votes, types = [], maxVotes = 100, onVoteSuccess }: RankingItemProps) {
  ...
  return (
    <div className="w-full glass-panel glass-panel-hover rounded-2xl p-3 sm:p-4 border border-slate-200 dark:border-white/10 flex items-center justify-between gap-3 sm:gap-4 relative overflow-hidden group">
      ...
      <div className="flex items-center gap-3 sm:gap-4 shrink-0">
        <span className="w-8 sm:w-10 text-center font-mono font-black text-base sm:text-xl text-slate-400 dark:text-slate-400 group-hover:text-amber-500 dark:group-hover:text-amber-400 transition-colors">
          #{rank}
        </span>

        <Link
          href={`/catalogue/${pokemon}`}
          className="relative w-12 h-12 sm:w-16 sm:h-16 rounded-xl bg-slate-100 dark:bg-slate-900/60 p-1 border border-slate-200 dark:border-white/10 overflow-hidden flex items-center justify-center shrink-0 group-hover:border-poke-cyan/50 transition-colors"
        >
          <Image
      ...
      <div className="flex flex-col flex-grow min-w-0">
        <div className="flex items-center gap-2">
          <Link
            href={`/catalogue/${pokemon}`}
            className="font-extrabold text-slate-900 dark:text-white text-base sm:text-lg capitalize truncate hover:text-poke-cyan transition-colors"
          >
            {pokemon}
          </Link>
      ...
      <div className="flex items-center gap-3 shrink-0">
        <div className="flex flex-col items-end">
          ...
          <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-mono">{t('rankings.votesLabel')}</span>
        </div>
