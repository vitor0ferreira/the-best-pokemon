...
import { Volume2, Sparkles, Trophy, ArrowLeft, RefreshCw, Zap, Shield } from 'lucide-react';
  ...
  if (isLoading) {
    return (
      <main className="flex-grow w-full max-w-4xl mx-auto px-4 py-16 flex flex-col items-center justify-center">
        ...
        <p className="text-slate-400 font-mono text-sm">{t('pokemonDetail.loading')}</p>
      </main>
    );
  }
    ...
    return (
      <main className="flex-grow w-full max-w-lg mx-auto px-4 py-16 flex flex-col items-center justify-center text-center">
        <div className="glass-panel rounded-3xl p-8 border border-red-500/30 text-slate-200">
          <h2 className="text-2xl font-bold text-red-400 mb-2">{t('pokemonDetail.notFoundTitle')}</h2>
          <p className="text-sm text-slate-400 mb-6">
            {t('pokemonDetail.notFoundDesc')}
          </p>
          <button
          ...
          >
            <RefreshCw className="w-4 h-4" /> {t('pokemonDetail.tryAgain')}
          </button>
        </div>
      </main>
    );
        ...
        <div className="md:col-span-5 flex flex-col items-center text-center">
          ...
          <div className="flex items-center gap-3 mt-4">
            ...
            >
              ...
              {isShiny ? t('pokemonDetail.shinyActive') : t('pokemonDetail.shinyView')}
            </button>
          ...
          >
            <Trophy className="w-5 h-5" /> {t('pokemonDetail.voteFor', { name: pokemonData.name })}
          </button>
        </div>
